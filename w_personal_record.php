...
// ==== ADD NEW WORKOUT (Personal Record) ====
if (isset($_POST['submit'])) {
    ...
    if (empty($day) || empty($wtype) || empty($wweight) || empty($wtime) || empty($wnote)) {
        echo json_encode(['status' => 'error', 'message' => 'Please fill all fields']);
        exit;
    }

    $sql = "INSERT INTO personal_records (day_of_week, exercise_name, max_weight_kg, record_date, W_note)
             VALUES ('$day', '$wtype', '$wweight', '$wtime', '$wnote')";
...
if (isset($_POST['delete_id'])) {
    ...
    $conn->query("DELETE FROM personal_records WHERE pr_id = '$id'"); 
    echo json_encode(['status' => 'deleted']);
    exit;
}

// ==== FETCH WORKOUTS (Fixed error prone logic) ====
$sql_fetch = "SELECT * FROM personal_records ORDER BY FIELD(day_of_week,
...
<div class="schedule-page">
    <div class="schedule-view">
        <h2>Save Your Max PRs</h2>
        <div id="schedule-container"></div>
    </div>

    <div class="add-workout-form">
        <h3>➕ Add PR Workout</h3>
        <form id="workoutForm">
            ...
            <input type="text" name="wtype" id="workout" placeholder="e.g. Bench Press" required>

            <label for="number">Max Weight (kg):</label>
            <input type="number" name="wweight" id="weight" required>

            <label for="time">Record Time:</label>
            <input type="time" name="wtime" id="time" required>

            <label for="notes">Notes/Reps:</label>
            <textarea id="notes" name="wnote" rows="3" placeholder="e.g. 5x5 reps, 2-minute rest..."></textarea>
...
function renderSchedule() {
    ...
    for (const day in grouped) {
        ...
        // Add a fitness emoji next to the day name
        ...
        grouped[day].forEach(w => {
            ...
            // Rendering logic using the PR table columns: exercise_name, max_weight_kg, record_date, W_note
            entry.innerHTML = `
                <div class="entry-header">
                    <p>
                        <span class="type">${w.exercise_name || 'N/A'}</span> 
                        <span class="weight">| Max: <strong>${w.max_weight_kg || 'N/A'}kg</strong></span> 
                        <span class="time">Time: ${w.record_date || 'N/A'}</span>
                    </p>
                    <button class="delete-btn" data-id="${w.pr_id}">Delete</button>
                </div>
                ${w.W_note ? `<p class="notes">Notes: ${w.W_note}</p>` : ''}
            `;
...
form.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        alert('An unexpected server error occurred. Please check the network tab or server logs.');
        return;
    }

    if (data.status === 'success') {
        // Push the new data structure (using PR column names) to the local array
        workouts.push({
            pr_id: data.pr_id,
            day_of_week: data.day_of_week, 
            exercise_name: formData.get('wtype'),
            max_weight_kg: formData.get('wweight'),
            record_date: formData.get('wtime'),
...
async function deleteWorkout(id) {
    ...
    formData.append('delete_id', id); // Sends the ID to the PHP delete logic
